...
(function (ngPlot) {
    var PlotCtrl = (function () {
        function PlotCtrl($scope) {
            ...
            $scope.drawAxes = function () {
                var xAxis = d3.svg.axis().scale($scope.xScale).ticks(5).orient("bottom");
                var yAxis = d3.svg.axis().scale($scope.yScale).ticks(5).orient("left");
                // Draw the axes
            ...
            $scope.drawPlot = function () {
                ...
                // disappear
                var clip = $scope.svg.append("defs").append("clipPath").attr("id", "plotArea").append("rect").attr("id", "clip-rect").attr("x", $scope.padding).attr("y", 0).attr("width", $scope.width - $scope.padding * 2).attr("height", $scope.height - $scope.padding);
                ...
                $scope.plot = chartBody.append("path").attr("d", pathGen(plotData)).attr("stroke", "blue").attr("stroke-width", 2).attr("fill", "none");
            };
            $scope.render = function () {
                ...
                var h = $scope.height;
                // Set up the scales
                $scope.xScale = d3.scale.linear().domain($scope.xDomain).range([p, w - p]);
                $scope.yScale = d3.scale.linear().domain($scope.yDomain).range([h - p, p]);
                $scope.drawAxes();
                $scope.drawPlot();
            ...
            $scope.drawLine = function (line) {
                ...
                var end = line.end;
                var drawnLine = $scope.svg.append("line").attr("x1", $scope.xScale(start[0])).attr("y1", $scope.yScale(start[1])).attr("x2", $scope.xScale(end[0])).attr("y2", $scope.yScale(end[1])).attr('stroke-width', sw).attr('stroke', color);
                return drawnLine;
            };
        ...
        PlotCtrl.prototype.addLine = function (line) {
            this.lines.push(line);
        };
        PlotCtrl.prototype.setOptions = function (opts) {
            if (opts) {
    ...
    angular.module('plotModule', []).directive('plot', plotDirective).directive('line', lineDirective);
})(ngPlot || (ngPlot = {}));
